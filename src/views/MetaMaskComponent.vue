<template>
  <div class="metamask-container">
    ...
    <button class="action-button" v-if="this.connected" @click="personal_sign">
      personal_sign
    </button>
    <button class="action-button" v-if="this.connected" @click="sendTransaction">
      Send transaction
    </button>
    <button v-if="this.connected" class="action-button" @click="switchChain('0xaa36a7')">
      Switch to Sepolia
    </button>
    <button class="action-button" v-if="this.connected" @click="switchChain('0x1')">
      Switch to Mainnet
    </button>
    <button class="action-button" v-if="this.connected" @click="addEthereumChain">
      Add Polygon
    </button>
  ...
  methods: {
    ...
    async onConnect() {
      try {
        ...
        //console.log('HESAP : ' + JSON.stringify(accounts));
        this.lastResponse = '';
